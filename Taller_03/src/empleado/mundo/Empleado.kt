...
class Empleado {
    ...
    fun calcularPrestaciones(): Double {
        var prestaciones = (calcularAntiguedad() * salario) / 12
     ...
     */
    fun tipo(): String {
        var tipoEmpleado = "Tipo de empleado"
        if (calcularAntiguedad() < 1){
            tipoEmpleado = "freshman"
        }
        else if (calcularAntiguedad() >= 1 && calcularAntiguedad() <= 3){
            tipoEmpleado = "sophomore"
        }
        else if (calcularAntiguedad() > 3 && calcularAntiguedad() < 10){
            tipoEmpleado = "junior"
        }
        else if (calcularAntiguedad() >= 10){
            tipoEmpleado = "senior"
        }
        return tipoEmpleado
    }
     ...
     * Este valor se calcula a partir del siete por ciento (7%) del salario por el número de
     * personas a cargo (dato de entrada).
     */
    fun bonificacion(numPersonasACargo: Int): Double {
        val bonificacion = (this.salario * 0.07) * numPersonasACargo
        return bonificacion
    }
}